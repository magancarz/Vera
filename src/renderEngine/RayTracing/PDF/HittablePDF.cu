...
__device__ HittablePDF::HittablePDF(curandState* curand_state, BVHTreeTraverser* intersection_accelerator_tree_traverser, Triangle** triangles, int num_of_triangles)
...
__device__ void HittablePDF::shuffleLightSources(Triangle** shuffled_light_sources) const
{
    for (int i = num_of_triangles - 1; i > 0; i--) {
        const int j = randomInt(curand_state, 0, num_of_triangles);
        Triangle* temp = shuffled_light_sources[i];
...
{
    auto shuffled_light_sources = static_cast<Triangle**>(malloc(num_of_triangles * sizeof(void*)));
    ...
    for (int i = 0; i < num_of_triangles; ++i)
    {
        Triangle* light_source = shuffled_light_sources[i];
