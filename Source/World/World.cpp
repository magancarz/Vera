...
void World::loadObjects(Device& device, const std::vector<std::shared_ptr<Material>>& available_materials)
{
    ...
    auto diffuse_light = std::make_shared<Material>(device, MaterialInfo{.color = glm::vec3{1.f, 1.f, 1.f}, .brightness = 5});
...
//    objects.emplace(left_light_plane->getID(), std::move(left_light_plane));
    ...
    auto specular = std::make_shared<Material>(device, MaterialInfo{.color = glm::vec3{1.f, 1.f, 1.f}, .fuzziness = 0.2f});
    auto dragon = std::make_shared<Object>(Object::createObject());
    dragon->transform_component.translation.y = 1.5f;
