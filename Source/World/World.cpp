...
{
    std::shared_ptr<Model> monkey_model = Model::createModelFromFile(device, "Resources/Models/cube_scene.obj");
    ...
    left_monkey->transform_component.rotation = {.0f, .0f, .0f};
...
//    left_monkey->material = available_materials[0];
    ...
    right_monkey->transform_component.rotation = {.0f, .0f, .0f};
