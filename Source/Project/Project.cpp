...
{
    std::string file_location = PathBuilder(location).append(project_info.project_name + PROJECT_FILE_EXTENSION).
                                                      build();
    ...
    if (file_stream.is_open())
    {
        ...
        LogSystem::log(LogSeverity::LOG, "Project named ", project_info.project_name,
                       " saved successfully at location ", file_location);
        return;
    }

    LogSystem::log(LogSeverity::ERROR, "Couldn't save project named ", project_info.project_name, " at location ",
                   file_location, "!");
}
...
ProjectInfo ProjectUtils::loadProject(const std::string& project_name, const std::string& location)
{
    ...
    if (file_stream.is_open())
    {
        ...
        loadProjectMetadata(line_from_file, file_stream, project_info);
